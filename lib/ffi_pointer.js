var Bindings = require('ffi_bindings')
  ...
  , Pointer = module.exports = Bindings.Pointer
  , util = require('util')
...
Object.keys(Util.NON_SPECIFIC_TYPES).forEach(function (type) {
  var method = Util.NON_SPECIFIC_TYPES[type]
    , suffix = Util.TYPE_TO_POINTER_METHOD_MAP[type]

  if (!suffix) {
    // No hard mapping, determine by size
    var size = Util.sizeOf(type)
      , szFunc = Util.SIZE_TO_POINTER_METHOD_MAP[size]
      , signed = type !== 'byte' && type != 'size_t' && type[0] != 'u'
    suffix = (signed ? '' : 'U') + szFunc
  }

  Util.TYPE_TO_POINTER_METHOD_MAP[type] = suffix
