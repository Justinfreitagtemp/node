...
exports.process = new ffi.DynamicLibrary()
...
exports.copyProtocolList = function copyProtocolList () {
  var num = new ffi.Pointer(ffi.TYPE_SIZE_MAP.uint32)
    , rtn = []
  ...
  for (var i=0; i<count; i++) {
    ...
    rtn.push(name)
    p = p.seek(ffi.TYPE_SIZE_MAP.pointer)
  }
  ffi.free(protos)
  return rtn
}
...
exports.copyIvarList = function copyIvarList (classPtr) {
  var numIvars = new ffi.Pointer(ffi.TYPE_SIZE_MAP.uint32)
    , rtn = []
...
exports.copyMethodList = function copyMethodList (classPtr) {
  var numMethods = new ffi.Pointer(ffi.TYPE_SIZE_MAP.uint32)
    , rtn = []
  ...
  for (var i=0; i<count; i++) {
    var cur = p.getPointer()
      , name = objc.sel_getname(exports.method_getName(cur))
    rtn.push(name)
    p = p.seek(ffi.TYPE_SIZE_MAP.pointer)
  }
  ffi.free(methods)
...
exports.copyMethodDescriptionList = function copyMethodDescriptionList (protocolPtr, required, instance) {
  ...
  for (var i=0; i<count; i++) {
    ...
    rtn.push(objc.sel_getname(cur.name))
 ...
 * TODO: Move this into a node-ffi helper class thingy.
 */
...
/**
 * Creates and/or returns an appropriately wrapped up 'objc_msgSendSuper'
