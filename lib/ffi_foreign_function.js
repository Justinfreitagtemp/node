var Bindings = require('ffi_bindings')
  ...
  , POINTER_SIZE = Bindings.POINTER_SIZE
...
function ForeignFunction (ptr, returnType, types, async) {
  ...
  var self = this
    ...
    , drefVal = Util.derefValuePtrFunc(returnType)
    , result = new Pointer(Util.sizeOf(returnType))
    , argsList = new Pointer(numArgs * POINTER_SIZE)
    , cif = new CIF(returnType, types)
    , caller = new Bindings.ForeignCaller(
        cif.getPointer()
      , ptr
      , argsList
      , result
  ...
  var argputf = types.map(function (type, i) {
    ...
    if (Util.isStructType(type)) {
      return function (val) {
        argPtr.putPointer(val.ref())
      }
    }

    var valPtr = new Pointer(Util.sizeOf(type))
    ...
    if (type == 'string') {
      ...
      var putCall = 'put' + Util.TYPE_TO_POINTER_METHOD_MAP[type]
      return function (val) {
        valPtr[putCall](val)
      }
    }
