...
exports.wrapValue = function wrapValue (val, type) {
  ...
  if (type.function_pointer) {
    if (type.type == '@?') {
      ...
      return method.createUnwrapperFunction(val, type)
    }
  }
  ...
  if (type == '@') {
    ...
    rtn = exports.wrapClass(val)
  } else if (type == ':') {
    rtn = core.sel_getname(val)
  } else if (type == 'B') {
    rtn = val ? true : false
  }
  if (rtn)
...
exports.unwrapValue = function unwrapValue (val, type) {
  ...
  if (type.function_pointer) {
    if (type.type == '@?') {
      ...
      return method.createWrapperPointer(val, type)
    }
  }
  // get the raw type from Type objects
  if (type.type) type = type.type
...
proto.msgSend = function msgSend (sel, args) {
  ...
  var argTypes = types[1]
    ...
    , unwrappedArgs = exports.unwrapValues([this, sel].concat(args), argTypes)
    , rtn
  console.log('H2');
  ...
  return exports.wrapValue(rtn, types[0])
}
...
proto.msgSendSuper = function msgSendSuper (sel, args) {
  ...
  var types = this._getTypes(sel, args)
    ...
    , unwrappedArgs = exports.unwrapValues([os, sel].concat(args), argTypes)
    , rtn
...
proto.getClass = function getClass () {
  return exports.wrapClass(core.object_getClass(this.pointer))
}
...
proto.setClass = function setClass (newClass) {
  return exports.wrapClass(core.object_setClass(this.pointer, newClass.pointer))
}
...
proto.ivar = function ivar (name, value) {
  ...
  if (arguments.length > 1) {
    ...
    var ivar = this.isClass
      ...
      , unwrapped = exports.unwrapValue(value, ivar.getTypeEncoding())
    ...
    return exports.wrapValue(ptr.getPointer(), core.ivar_getTypeEncoding(ivar))
  }
}
...
function getClass (className) {
  ...
  if (rtn) {
    ...
    rtn = exports.wrapClass(pointer, className)
  }
  return rtn
}
...
proto.addMethod = function addMethod (selector, type, func) {
  var parsed = types.parse(type)
    , selRef = core.toSEL(selector)
    , funcPtr = method.createWrapperPointer(func, parsed)
  // flatten the type
...
proto.getInstanceMethod = function getInstanceMethod (sel) {
  return method.wrap(this._getInstanceMethod(core.toSEL(sel)))
}
...
proto.getClassMethod = function getClassMethod (sel) {
  return method.wrap(this._getClassMethod(core.toSEL(sel)))
}
