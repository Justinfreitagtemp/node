...
function Callback (typedata, func) {
  ...
  this._cif   = new CIF(retType, types)
  this._info  = new Bindings.CallbackInfo(this._cif.getPointer(), function (retval, params) {
    var pptr = params.clone()
    var args = types.map(function (type) {
      return Util.derefValuePtr(type, pptr.getPointer(true))
    })
    ...
    if (retType !== 'void') {
      retval['put' + Util.TYPE_TO_POINTER_METHOD_MAP[retType]](result)
    }
  })
