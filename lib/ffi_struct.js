...
function Struct () {
  ...
  function read (ptr, name) {
    ...
    if (Util.isStructType(info.type)) {
      return new info.type(fptr)
    } else if (info.type == 'string') {
      return fptr.getPointer().getCString()
    } else {
      return fptr['get' + Util.TYPE_TO_POINTER_METHOD_MAP[info.type]]()
    }
  }
  ...
  function write (ptr, name, val) {
    ...
    if (Util.isStructType(info.type)) {
      ...
      if (typeof val == 'undefined' || val === null) {
        return fptr.putPointer(Pointer.NULL)
      }
      var len = Buffer.byteLength(val, 'utf8')
      var strPtr = new Pointer(len+1)
      strPtr.putCString(val)
      fptr.putPointer(strPtr)
    } else {
      return fptr['put' + Util.TYPE_TO_POINTER_METHOD_MAP[info.type]](val)
    }
  }
  ...
  for (var i=0, len=fields.length; i<len; i++) {
    ...
    var stype   = Util.isStructType(type)
      , sz      = Util.sizeOf(type)
  ...
  var constructor = function (arg, data) {
    ...
    if (Pointer.isPointer(arg)) {
      this.pointer = arg
      arg = data
    } else {
      this.pointer = new Pointer(struct.size)
    }
    if (arg) {
      for (var key in arg) {
  ...
  constructor._ffiType = function ffiType () {
    ...
    t.elements = new Pointer(Bindings.POINTER_SIZE * (numProps+1))
    var tptr = t.elements.clone()
    for (var i=0; i<numProps; i++) {
      var prop = props[propNames[i]]
      tptr.putPointer(Util.ffiTypeFor(prop.type), true)
    }
    // Final NULL pointer to terminate the Array
    tptr.putPointer(Pointer.NULL)
