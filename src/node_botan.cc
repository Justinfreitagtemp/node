...
void pk::DoingGenerate(uv_work_t *request) {
  ...
  try {
    ...
    baton->publicKey = baton->privateKey;
  }
...
void pk::DoingLoadPrivateKey(uv_work_t *request) {
  ...
  try {
    ...
    std::auto_ptr<PBKDF> firstPassPbkdf(get_pbkdf(PBKDF_TYPE));
    OctetString firstPassKey = firstPassPbkdf->derive_key(PBKDF_SIZE,
      *baton->passphrase, salt.begin(), PBKDF_SIZE, PBKDF_ITERATIONS);
    std::auto_ptr<PBKDF> secondPassPbkdf(get_pbkdf(PBKDF_TYPE));
    OctetString secondPassKey = secondPassPbkdf->derive_key(PBKDF_SIZE,
      *baton->passphrase, salt.begin() + PBKDF_SIZE, PBKDF_SIZE, PBKDF_ITERATIONS);
    SecureVector<byte> ivString = base64_decode(*baton->iv);
    InitializationVector firstPassIv(ivString.begin(), IV_SIZE);
    InitializationVector secondPassIv(ivString.begin() + IV_SIZE, IV_SIZE);
    Pipe pipe(new Chain(new Base64_Decoder, get_cipher(SECOND_PASS_CIPHER_TYPE,
      secondPassKey, secondPassIv, DECRYPTION), get_cipher(FIRST_PASS_CIPHER_TYPE,
      firstPassKey, firstPassIv, DECRYPTION)));
...
void PrivateKey::DoingToString(uv_work_t *request) {
  ...
  try {
    ...
    OctetString secondPassKey = secondPassPbkdf->derive_key(PBKDF_SIZE,
      *baton->passphrase, salt.begin() + PBKDF_SIZE, PBKDF_SIZE, PBKDF_ITERATIONS);
    InitializationVector firstPassIv(rng, IV_SIZE);
    InitializationVector secondPassIv(rng, IV_SIZE);
    Pipe pipe(new Chain(get_cipher(FIRST_PASS_CIPHER_TYPE, firstPassKey, firstPassIv, ENCRYPTION),
      get_cipher(SECOND_PASS_CIPHER_TYPE, secondPassKey, secondPassIv, ENCRYPTION), new Base64_Encoder));
    pipe.process_msg(PKCS8::BER_encode(*baton->privateKey));

    baton->salt = new std::string(base64_encode(salt));
    baton->iv = new std::string(base64_encode((firstPassIv + secondPassIv).bits_of()));
...
PublicKey::PublicKey(Handle<Object> target, Public_Key* publicKey) {
...
PrivateKey::PrivateKey(Handle<Object> target, Private_Key* privateKey) {
...
NODE_MODULE(node_botan, node_botan::init);
